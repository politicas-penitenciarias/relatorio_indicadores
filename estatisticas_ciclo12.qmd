...
- **Departamento de Polícia Civil:** Alagoas, Ceará, Pará e Paraíba;

- **Polícia Militar:** Sergipe;

- **Corpo de Bombeiro Militar:** Alagoas, Amazonas, Amapá, Maranhão e Rio Grande do Norte.
...
A [seção @sec-populacao-prisional-ciclo12] trata dos indicadores que definem a população prisional brasileira considerando os diversos tipos de estabelecimentos, regimes de custódia, gênero dos custodiados, modalidade de cumprimento de pena, além dos aspectos legais vinculados.
...
A [subseção @sec-indicadores-populacionais-brasil-ciclo12] trata dos indicadores populacionais em âmbito nacional de forma tabular e gráfica em valores percentuais (relativos) e absolutos. Realiza-se o estudo da população prisional considerando as modalidades de custódia e gêneros.
...
A tabela acima informa a quantidade de **`r format(t1$Total[2], big.mark = ".", decimal.mark = ",")`** equipamentos de monitoramento contratados para uso em todo o Brasil. Desse total, **`r format(index_pop$Total[3], big.mark = ".", decimal.mark = ",")`** estão em utilização, conforme [subseção @sec-indicadores-populacionais-brasil-ciclo12].
...
A primeira parte deste tópico ([subseção @sec-vagas-brasil-ciclo12]) trata das taxas supracitadas em âmbito nacional calculadas sobre o somatório das populações prisionais estaduais. Dessa forma, retrata a realidade ampla do sistema penitenciário nacional.
...
**Taxa de aprisionamento:** Dada pela relação entre a **quantidade pessoas privadas de liberdade**[^estatisticas_ciclo12-2] cumprindo pena em estabelecimentos prisionais e a quantidade de habitantes no país, região ou estado para cada 100.000 habitantes.

[^estatisticas_ciclo12-2]: Quantitativo de pessoas cumprindo pena nas modalidades de custódia em estabelecimentos prisionais, carceragens e batalhões de polícia. Dessa maneira, exclui-se dessa variável as pessoas em prisão domiciliar.

$$
Taxa \ de \ aprisionamento  = \left(\dfrac{Número \ de \ pessoas \ privadas \ de \ liberdade}{Quantidade \ de \ habitantes}\right) \times 100.000
$$

**Taxa de penalização:** Dada pela relação entre a **população prisional geral** [^estatisticas_ciclo12-3] e a quantidade de habitantes no país, região ou estado para cada 100.000 habitantes.

[^estatisticas_ciclo12-3]: A **população prisional geral** é formada por pessoas cumprindo pena em todas as modalidades de custódia, sendo custodiados em: estabelecimentos prisionais, carceragens e batalhões de polícia, prisão domiciliar e prisão domiciliar com monitoramento eletrônico.
