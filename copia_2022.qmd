# Parâmetros e indicadores - Ciclo 13 - Dez/2022 {#sec-parametros-indicadores-ciclo13}
...
Este capítulo traz as informações pormenorizadas acerca `r str_c(vetor_filtro_ciclo,"°")` do Levantamento Nacional de Informações Penitenciárias - Infopen - referente ao `r vetor_filtro_semestre` semestre de `r vetor_filtro_ano`.
...
populacao_prisional_sexo <- 
  ...
  group_by(modalidade,ambito,sexo, variavel) |> 
  summarise(
...
populacao_prisional_sexo1 <- 
  populacao_prisional_sexo |> 
  filter(variavel == "População") |> 
  select(-variavel)
...
Para este ciclo foram contabilizadas `r sum(populacao_prisional_sexo1%total)` pessoas cumprindo algum tipo de medida punitiva em estabelecimentos penitenciários estadual ou federal.

Nesse sentido, a tabela [@tbl-indicadores_populacionais_ciclo13] lista os indicadores populacionais do sistema.

```{r}
#| label: tbl-indicadores_populacionais_ciclo13
#| tbl-cap: >
#|   Tipificação dos processos recebidos pela Ouvidoria Nacional dos Serviços Penais.
    

t1 <- populacao_prisional_sexo
names(t1) <- c("Modalidade","Âmbito","Feminino","Masculino","Total")
...
Para as demandas enquadradas como **denúncia**, obteve-se `r sum(encaminhamento2$qtd)` encaminhamentos para outros órgãos. [^copia_2022-2].

[^copia_2022-2]: A coleta de dados para esse quesito iniciou-se em dezembro de 2021.
